...
:root{--gold:#F0B429;--gold-dim:#7a5c0a;--black:#070707;--s1:#111;--s2:#181818;--border:#222;--border2:#2a2a2a;--text:#E8E8E8;--dim:#666;--green:#00C896;--red:#FF4560;--purple:#6C5CE7;--blue:#0984e3;}
...
/* NOTIFICATIONS */
.notif-item{display:flex;align-items:flex-start;gap:1rem;background:var(--s1);border:1px solid var(--border);border-radius:12px;padding:1rem 1.2rem;margin-bottom:.6rem;}
...
.funded-badge-inline{display:inline-block;background:linear-gradient(135deg,var(--gold),#c88a00);color:#000;font-family:'Bebas Neue',sans-serif;font-size:.9rem;letter-spacing:2px;padding:4px 16px;border-radius:20px;margin-top:8px;}
...
/* CERT */
.cert-container{background:linear-gradient(135deg,#0a0800,#111,#0a0800);border:2px solid var(--gold-dim);border-radius:20px;padding:3rem;text-align:center;position:relative;overflow:hidden;max-width:700px;margin:0 auto;}
.cert-container::before{content:'';position:absolute;top:0;left:0;right:0;bottom:0;background:repeating-linear-gradient(45deg,transparent,transparent 40px,rgba(240,180,41,.02) 40px,rgba(240,180,41,.02) 41px);pointer-events:none;}
.cert-title{font-family:'Bebas Neue',sans-serif;font-size:3rem;letter-spacing:6px;background:linear-gradient(135deg,#F0B429,#fff5a0,#F0B429);-webkit-background-clip:text;-webkit-text-fill-color:transparent;margin-bottom:.5rem;}
.cert-badge-inner{font-family:'Bebas Neue',sans-serif;font-size:1.2rem;letter-spacing:3px;background:var(--gold);color:#000;padding:8px 24px;border-radius:30px;display:inline-block;margin:1.5rem 0;}

/* CHAT */
.chat-container{background:var(--s1);border:1px solid var(--border);border-radius:16px;overflow:hidden;margin-bottom:1rem;}
...
.chat-avatar{width:28px;height:28px;border-radius:50%;font-size:.75rem;display:flex;align-items:center;justify-content:center;flex-shrink:0;}
...
/* REFERRAL */
.ref-code-box{background:var(--s2);border:1px solid var(--gold-dim);border-radius:10px;padding:1rem 1.4rem;display:flex;align-items:center;justify-content:space-between;margin:1rem 0;}
.ref-code{font-family:'JetBrains Mono',monospace;font-size:1.4rem;color:var(--gold);font-weight:700;letter-spacing:4px;}
.ref-stats{display:grid;grid-template-columns:repeat(3,1fr);gap:1rem;margin:1rem 0;}

/* MARKET TICKER */
.ticker-wrap{overflow:hidden;background:var(--s1);border-bottom:1px solid var(--border);padding:.5rem 0;margin-bottom:1.5rem;}
...
.ticker-price{font-family:'JetBrains Mono',monospace;color:var(--dim);}
.ticker-chg.up{color:var(--green);}
.ticker-chg.dn{color:var(--red);}

/* ORDERBOOK */
.ob-row{display:grid;grid-template-columns:1fr 1fr 1fr;padding:.3rem .8rem;font-size:.78rem;font-family:'JetBrains Mono',monospace;}
.ob-bid{color:var(--green);}
.ob-ask{color:var(--red);}
.ob-vol{color:#444;text-align:right;}

/* HEATMAP */
.heatmap-grid{display:grid;grid-template-columns:repeat(5,1fr);gap:.5rem;margin-top:1rem;}
.hmap-cell{border-radius:8px;padding:.8rem .5rem;text-align:center;cursor:default;}
.hmap-sym{font-size:.72rem;font-weight:700;color:#E8E8E8;}
.hmap-chg{font-size:.82rem;font-family:'JetBrains Mono',monospace;margin-top:2px;}

/* SCANNER */
.scan-row{display:flex;align-items:center;justify-content:space-between;background:var(--s1);border:1px solid var(--border);border-radius:10px;padding:.8rem 1.2rem;margin-bottom:.5rem;}
.scan-sym{font-weight:700;font-size:.88rem;}
.scan-signal{font-size:.65rem;font-weight:800;letter-spacing:1.5px;padding:3px 10px;border-radius:20px;}
.scan-signal.bull{background:rgba(0,200,150,.12);color:var(--green);}
.scan-signal.bear{background:rgba(255,69,96,.12);color:var(--red);}
.scan-signal.neut{background:rgba(240,180,41,.12);color:var(--gold);}

/* STEPS */
.steps-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:1.5rem;margin:3rem 0;}
.step-card{background:var(--s1);border:1px solid var(--border);border-radius:16px;padding:1.8rem 1.5rem;text-align:center;}
.step-num{font-family:'Bebas Neue',sans-serif;font-size:3rem;color:var(--gold);-webkit-text-fill-color:var(--gold);line-height:1;margin-bottom:.5rem;}
.step-title{font-family:'Bebas Neue',sans-serif;font-size:1.1rem;letter-spacing:2px;color:#E8E8E8;margin-bottom:.5rem;}
...
/* TRADE ANALYTICS */
.analytics-card{background:var(--s1);border:1px solid var(--border);border-radius:14px;padding:1.4rem;margin-bottom:1rem;}
.streak-badge{display:inline-flex;align-items:center;gap:.4rem;padding:4px 12px;border-radius:20px;font-size:.72rem;font-weight:700;}
.streak-win{background:rgba(0,200,150,.12);color:var(--green);}
.streak-loss{background:rgba(255,69,96,.12);color:var(--red);}

/* WATCHLIST */
.wl-row{display:flex;align-items:center;justify-content:space-between;padding:.7rem .5rem;border-bottom:1px solid var(--border2);}
...
/* STREAMLIT OVERRIDES */
.stButton>button{background:var(--gold)!important;color:#000!important;font-weight:700!important;border:none!important;border-radius:8px!important;font-family:'DM Sans',sans-serif!important;letter-spacing:1px!important;white-space:nowrap!important;}
...
# Simulated market data for live ecosystem
MARKET_DATA = {
...
# ─── SESSION STATE ──────────────────────────────────────────────
for k,v in [
    ("user",None),("page","home"),("theme","dark"),
    ("notifications",[]),("chat_history",[]),
    ("watchlist",["NIFTY","BANKNIFTY","BTCUSDT","GOLD"]),
    ("open_positions",[]),("alerts",[]),
]:
...
def push_notification(uid, icon, title, msg):
    ...
    notifs.insert(0,{"uid":uid,"icon":icon,"title":title,"msg":msg,"time":datetime.utcnow().strftime("%H:%M"),"unread":True})
...
def get_simulated_price(symbol, base_price=None):
    ...
    return round(base_price * (1 + move), 2)
...
def render_certificate(name, plan, capital, pnl_pct, days, date_str):
    ...
    <div class="cert-container">
      <div style="font-size:4rem;margin:0 0 .5rem;">🏆</div>
      <div class="cert-title">CERTIFICATE OF ACHIEVEMENT</div>
      <div style="font-size:.75rem;color:#555;letter-spacing:3px;text-transform:uppercase;margin-bottom:2rem;">AKFunded Prop Trading Challenge</div>
      <div style="font-size:.82rem;color:#555;margin-bottom:.5rem;">This certifies that</div>
      <div style="font-family:'Bebas Neue',sans-serif;font-size:2rem;color:#E8E8E8;letter-spacing:3px;margin:1rem 0;">{name.upper()}</div>
      <div style="font-size:.85rem;color:#666;margin-bottom:1.5rem;">has successfully completed the</div>
      <div class="cert-badge-inner">⚡ {plan.upper()} CHALLENGE — {cap_str}</div>
      ...
      <div style="font-size:.7rem;color:#333;letter-spacing:2px;text-transform:uppercase;margin-top:1.5rem;border-top:1px solid #222;padding-top:1rem;">
...
def nav():
    ...
    if logged_in:
        ...
        st.markdown("""<style>.stButton>button{font-size:0.72rem!important;padding:0.4rem 0.6rem!important;letter-spacing:0.5px!important;}</style>""", unsafe_allow_html=True)
...
def render_live_ticker():
    ...
    for sym, data in MARKET_DATA.items():
        ...
        sign = "+" if chg >= 0 else ""
        items += f'<div class="ticker-item"><span class="ticker-sym">{sym}</span><span class="ticker-price">{data["price"]:,.2f}</span><span class="ticker-chg {cls}">{sign}{chg:.2f}%</span></div>'
    # Duplicate for seamless loop
    all_items = items + items
    st.markdown(f"""
    <div class="ticker-wrap">
      <div class="ticker-inner">{all_items}</div>
    </div>""", unsafe_allow_html=True)

def render_market_heatmap():
    """Render a color-coded heatmap of market performance"""
    cells = ""
    for sym, data in MARKET_DATA.items():
        chg = data["change"]
        if chg > 1.5:   bg = "rgba(0,200,150,.35)"
        elif chg > 0.5: bg = "rgba(0,200,150,.18)"
        elif chg > 0:   bg = "rgba(0,200,150,.08)"
        elif chg > -0.5:bg = "rgba(255,69,96,.08)"
        elif chg > -1.5:bg = "rgba(255,69,96,.18)"
        else:            bg = "rgba(255,69,96,.35)"
        sign = "+" if chg >= 0 else ""
        col  = "var(--green)" if chg >= 0 else "var(--red)"
        cells += f'<div class="hmap-cell" style="background:{bg};"><div class="hmap-sym">{sym}</div><div class="hmap-chg" style="color:{col};">{sign}{chg:.2f}%</div></div>'
    st.markdown(f'<div class="heatmap-grid">{cells}</div>', unsafe_allow_html=True)

def render_signal_scanner():
    """Generate and display buy/sell signals for symbols"""
    signals = []
    for sym, data in MARKET_DATA.items():
        chg = data["change"]
        vol = data["vol"]
        if chg > 1.5 and vol in ["High","Very High"]:
            sig = "STRONG BUY"; cls = "bull"
        elif chg > 0.3:
            sig = "BUY";        cls = "bull"
        elif chg < -1.5 and vol in ["High","Very High"]:
            sig = "STRONG SELL";cls = "bear"
        elif chg < -0.3:
            sig = "SELL";       cls = "bear"
        else:
            sig = "NEUTRAL";    cls = "neut"
        rsi = random.randint(30,70)
        signals.append({"sym":sym,"sig":sig,"cls":cls,"chg":chg,"rsi":rsi,"vol":vol})

    for s in signals:
        sign = "+" if s["chg"] >= 0 else ""
        col  = "var(--green)" if s["chg"] >= 0 else "var(--red)"
        st.markdown(f"""
        <div class="scan-row">
          <div>
            <div class="scan-sym">{s['sym']}</div>
            <div style="font-size:.7rem;color:#555;">RSI: {s['rsi']} &nbsp;·&nbsp; Vol: {s['vol']}</div>
          </div>
          <div style="text-align:right;">
            <div class="scan-signal {s['cls']}">{s['sig']}</div>
            <div style="font-size:.72rem;color:{col};font-family:'JetBrains Mono',monospace;margin-top:4px;">{sign}{s['chg']:.2f}%</div>
          </div>
        </div>""", unsafe_allow_html=True)
...
if st.session_state.page == "home":
    ...
    <div style="text-align:center;margin:3rem 0 1rem;">
      <div style="font-family:'Bebas Neue',sans-serif;font-size:2rem;letter-spacing:4px;color:#E8E8E8;">LIVE MARKET SNAPSHOT</div>
      <div style="color:#555;font-size:.88rem;">Real-time simulated prices across all tradeable instruments</div>
    </div>""", unsafe_allow_html=True)
    ...
    with col1:
        st.markdown("""
        <div style="background:var(--s1);border:2px solid var(--border);border-radius:18px;padding:2rem;">
    ...
    with col2:
        st.markdown("""
        <div style="background:linear-gradient(160deg,#0d0b1a,var(--s1));border:2px solid #6C5CE7;border-radius:18px;padding:2rem;position:relative;overflow:hidden;">
    ...
    <div class="steps-grid">
      <div class="step-card"><div class="step-num">01</div><div class="step-title">CHOOSE A PLAN</div><div class="step-desc">Pick One-Phase or Two-Phase. Select your account size from $10K to $200K. Pay once, no subscriptions.</div></div>
      <div class="step-card"><div class="step-num">02</div><div class="step-title">PASS THE CHALLENGE</div><div class="step-desc">Hit the profit target while respecting drawdown limits. Trade Forex, Crypto, Indices, Commodities.</div></div>
      <div class="step-card"><div class="step-num">03</div><div class="step-title">GET FUNDED</div><div class="step-desc">Pass verification and receive your funded account. Real capital, real rules, real payouts within 24 hours.</div></div>
      <div class="step-card"><div class="step-num">04</div><div class="step-title">SCALE UP</div><div class="step-desc">Keep up to 90% of profits. Scale your account up to $2,000,000 as you consistently perform.</div></div>
    </div>""", unsafe_allow_html=True)
            ...
            if st.button("CREATE ACCOUNT →", use_container_width=True, key="su_btn"):
                if not name or not email2 or not pwd2:
                    ...
                    try:
                        ...
                        uid = res.user.id
                        db_save_profile(uid, name, email2, country.split(" ",1)[-1])
...
# ══════════════════════════════════════════════════════════════
    ...
    <div style="text-align:center;margin-bottom:2rem;">
      <div style="font-family:'Bebas Neue',sans-serif;font-size:2rem;letter-spacing:3px;color:#E8E8E8;">CHOOSE YOUR CHALLENGE, {name.upper()} 👋</div>
      <div style="color:#666;font-size:.88rem;margin-top:.3rem;">One-time fee · No subscription · Trade your way to funding</div>
    </div>""", unsafe_allow_html=True)
    ...
    def render_plan_card(plan, phase):
        ...
        st.markdown(f"""
        <div style="background:var(--s1);{bg}border:2px solid {border};border-radius:18px;padding:1.8rem;position:relative;overflow:hidden;">
          {badge}
          <div style="font-family:'Bebas Neue',sans-serif;font-size:1.2rem;letter-spacing:3px;color:#555;margin-bottom:.3rem;">ACCOUNT SIZE</div>
          <div style="font-family:'Bebas Neue',sans-serif;font-size:2.8rem;color:var(--gold);letter-spacing:2px;line-height:1;margin-bottom:1.2rem;">{plan['name']}</div>
          <ul class="plan-rules" style="margin-bottom:1.2rem;">{rules_html}</ul>
          <div style="border-top:1px solid var(--border);padding-top:1rem;">
            <div style="font-size:.65rem;color:#555;letter-spacing:1.5px;text-transform:uppercase;">One-time fee</div>
            <div style="font-family:'Bebas Neue',sans-serif;font-size:2.2rem;color:#E8E8E8;letter-spacing:2px;">${plan['price']}</div>
          </div>
        </div>""", unsafe_allow_html=True)
        ...
        if st.button(f"START ${plan['price']} CHALLENGE", key=f"buy_{plan['slug']}", use_container_width=True):
            ...
            <script>
              var rzp=new Razorpay({{key:"{rz}",amount:{inr_amount},currency:"INR",name:"AKFunded",
              description:"{plan['name']} {['One','Two'][phase-1]}-Phase Challenge",
              prefill:{{email:"{st.session_state.user.get('email','')}"}},
              theme:{{color:"#F0B429"}},
              handler:function(r){{alert("✅ Payment successful! Challenge activated.");}}}});rzp.open();
    ...
    with tab1:
        st.markdown("""<div style="background:rgba(240,180,41,.05);border:1px solid var(--gold-dim);border-radius:12px;padding:1rem 1.5rem;margin:.5rem 0 1.5rem;">
          <span style="font-size:.82rem;color:#888;">✅ Single phase &nbsp;·&nbsp; ✅ Faster funding &nbsp;·&nbsp; ✅ 80% profit split &nbsp;·&nbsp; ✅ No time limit &nbsp;·&nbsp; ✅ 24hr payouts</span>
        </div>""", unsafe_allow_html=True)
        cols = st.columns(5)
    ...
    with tab2:
        st.markdown("""<div style="background:rgba(108,92,231,.05);border:1px solid #6C5CE7;border-radius:12px;padding:1rem 1.5rem;margin:.5rem 0 1.5rem;">
          <span style="font-size:.82rem;color:#888;">✅ 90% profit split &nbsp;·&nbsp; ✅ Lower fees &nbsp;·&nbsp; ✅ Two verification phases &nbsp;·&nbsp; ✅ Scale to $2M &nbsp;·&nbsp; ✅ 24hr payouts</span>
        </div>""", unsafe_allow_html=True)
        cols = st.columns(5)
    ...
    # Compare table
    st.markdown("""<br><div style="font-family:'Bebas Neue',sans-serif;font-size:1.2rem;letter-spacing:3px;color:#555;margin-bottom:1rem;">COMPARE PROGRAMS</div>
    <div style="background:var(--s1);border:1px solid var(--border);border-radius:14px;overflow:hidden;">
      <div style="display:grid;grid-template-columns:2fr 1fr 1fr;padding:.8rem 1.2rem;background:var(--s2);font-size:.65rem;color:#555;letter-spacing:1.5px;text-transform:uppercase;">
    ...
    for feat,v1,v2 in rows:
        st.markdown(f"""<div style="display:grid;grid-template-columns:2fr 1fr 1fr;padding:.7rem 1.2rem;border-top:1px solid var(--border2);font-size:.82rem;">
...
# ══════════════════════════════════════════════════════════════
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1.5rem;">
      ...
      <div style="display:flex;gap:.8rem;align-items:center;">
        ...
        <div style="background:var(--s1);border:1px solid var(--border);color:#666;font-size:.7rem;padding:7px 14px;border-radius:20px;">
    ...
    with col_trade:
        ...
        sim_base = MARKET_DATA.get(t_sym, {}).get("price", 100.0)
        live_px  = get_simulated_price(t_sym, sim_base)
        ...
        <div style="background:var(--s2);border:1px solid {ec};border-radius:10px;padding:1rem;margin:.8rem 0;text-align:center;">
          ...
          <div style="font-size:.72rem;color:{ec};margin-top:2px;">{es}{roi:.2f}% ROI &nbsp;·&nbsp; R:R = 1:{rr:.1f}</div>
          <div style="font-size:.7rem;color:var(--red);margin-top:4px;">Risk: ₹{risk_trade:,.0f}</div>
        </div>""", unsafe_allow_html=True)

        # Live price display
        st.markdown(f"""
        <div style="background:rgba(240,180,41,.06);border:1px solid var(--gold-dim);border-radius:8px;padding:.6rem 1rem;margin-bottom:.8rem;text-align:center;">
          <div style="font-size:.6rem;color:#555;letter-spacing:2px;">SIMULATED LIVE PRICE</div>
          <div style="font-family:'JetBrains Mono',monospace;font-size:1.2rem;color:var(--gold);">₹{live_px:,.2f}</div>
        </div>""", unsafe_allow_html=True)
        ...
        if st.button("⚡ EXECUTE TRADE", use_container_width=True, key="exec"):
            ...
            if new_pct >= r["target"] and new_days >= r["min_days"]:
                ...
                push_notification(uid,"🏆","CHALLENGE PASSED!",f"You hit +{r['target']}% profit target. Funded badge unlocked!")
                ...
                push_notification(uid,"❌","Challenge Failed",f"Max total loss of -{r['total_loss']}% hit.")
                st.error("❌ Challenge FAILED — max total loss hit.")
            else:
                push_notification(uid,"⚡","Trade Executed",f"{ttype} {t_sym} — P&L: {es}₹{est:,.0f}")
                if est >= 0:
                    st.success(f"✅ Trade done! P&L: {es}₹{est:,.0f}")
                else:
                    st.warning(f"⚠️ Trade done! P&L: ₹{est:,.0f}")
    ...
    if trades:
        ...
        for t in trades:
            ...
            st.markdown(f'<div class="t-row"><span style="font-weight:600;">{t.get("symbol","")} <span style="font-size:.65rem;color:#444;">{dt}</span></span>{tag}<span>₹{t.get("entry_price",0):,.1f}</span><span>₹{t.get("exit_price",0):,.1f}</span><span>{t.get("quantity",0)}</span><span style="color:{pc3};font-family:\'JetBrains Mono\',monospace;font-weight:700;">{ps3}₹{p:,.0f}</span></div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
    else:
...
# ══════════════════════════════════════════════════════════════
# MARKETS (NEW — real-time trading ecosystem)
# ══════════════════════════════════════════════════════════════
    ...
    sec("📡 LIVE MARKETS","Real-time simulated prices, signals, orderbook & watchlist")

    tab_hm, tab_sc, tab_ob, tab_wl = st.tabs(["  🌡️ HEATMAP  ","  📡 SIGNAL SCANNER  ","  📖 ORDER BOOK  ","  ⭐ WATCHLIST  "])

    with tab_hm:
        st.markdown('<div style="font-family:\'Bebas Neue\',sans-serif;font-size:1rem;letter-spacing:2px;color:#555;margin-bottom:.5rem;">MARKET HEATMAP — ALL INSTRUMENTS</div>', unsafe_allow_html=True)
        render_market_heatmap()
        st.markdown("<br>", unsafe_allow_html=True)
        ...
        <div style="background:var(--s1);border:1px solid var(--border);border-radius:12px;overflow:hidden;">
          <div style="display:grid;grid-template-columns:1.5fr 1fr 1fr 1fr 1fr;padding:.8rem 1.2rem;background:var(--s2);font-size:.65rem;color:#555;letter-spacing:1.5px;text-transform:uppercase;">
            <span>SYMBOL</span><span>PRICE</span><span>CHANGE</span><span>VOLUME</span><span>SIGNAL</span>
          </div>""", unsafe_allow_html=True)
        for sym, data in MARKET_DATA.items():
            chg  = data["change"]
            col  = "var(--green)" if chg >= 0 else "var(--red)"
            ...
            st.markdown(f"""
            <div style="display:grid;grid-template-columns:1.5fr 1fr 1fr 1fr 1fr;padding:.7rem 1.2rem;border-top:1px solid var(--border2);align-items:center;">
              <span style="font-weight:700;color:#E8E8E8;">{sym}</span>
              <span style="font-family:'JetBrains Mono',monospace;">₹{data['price']:,.2f}</span>
              <span style="color:{col};font-weight:700;">{sign}{chg:.2f}%</span>
              <span style="font-size:.75rem;color:#555;">{data['vol']}</span>
              <span class="scan-signal {sc}" style="width:fit-content;">{sig}</span>
            </div>""", unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)

    with tab_sc:
        st.markdown('<div style="font-family:\'Bebas Neue\',sans-serif;font-size:1rem;letter-spacing:2px;color:#555;margin-bottom:1rem;">AI SIGNAL SCANNER — LIVE ALERTS</div>', unsafe_allow_html=True)
        render_signal_scanner()
        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown("""
        <div style="background:rgba(240,180,41,.05);border:1px solid var(--gold-dim);border-radius:10px;padding:.8rem 1.2rem;">
          <div style="font-size:.78rem;color:#666;">⚠️ Signals are generated using simulated price momentum + volume analysis. Always use proper risk management.</div>
        </div>""", unsafe_allow_html=True)

    with tab_ob:
        st.markdown('<div style="font-family:\'Bebas Neue\',sans-serif;font-size:1rem;letter-spacing:2px;color:#555;margin-bottom:1rem;">SIMULATED ORDER BOOK</div>', unsafe_allow_html=True)
        ob_sym = st.selectbox("Select Symbol", list(MARKET_DATA.keys()), key="ob_sym")
        base   = MARKET_DATA[ob_sym]["price"]

        col_bid, col_ask = st.columns(2)
        with col_bid:
            st.markdown('<div style="font-family:\'Bebas Neue\',sans-serif;font-size:.9rem;letter-spacing:2px;color:var(--green);margin-bottom:.5rem;">BIDS</div>', unsafe_allow_html=True)
            st.markdown('<div style="background:var(--s1);border:1px solid var(--border);border-radius:10px;overflow:hidden;"><div class="ob-row" style="color:#555;font-size:.65rem;letter-spacing:1.5px;text-transform:uppercase;border-bottom:1px solid var(--border);"><span>PRICE</span><span>SIZE</span><span>TOTAL</span></div>', unsafe_allow_html=True)
            for i in range(8):
                px  = round(base - (i+1)*base*0.0005, 2)
                sz  = random.randint(100,5000)
                tot = px * sz
                st.markdown(f'<div class="ob-row"><span class="ob-bid">₹{px:,.2f}</span><span style="color:#E8E8E8;">{sz:,}</span><span class="ob-vol">₹{tot:,.0f}</span></div>', unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)

        with col_ask:
            st.markdown('<div style="font-family:\'Bebas Neue\',sans-serif;font-size:.9rem;letter-spacing:2px;color:var(--red);margin-bottom:.5rem;">ASKS</div>', unsafe_allow_html=True)
            st.markdown('<div style="background:var(--s1);border:1px solid var(--border);border-radius:10px;overflow:hidden;"><div class="ob-row" style="color:#555;font-size:.65rem;letter-spacing:1.5px;text-transform:uppercase;border-bottom:1px solid var(--border);"><span>PRICE</span><span>SIZE</span><span>TOTAL</span></div>', unsafe_allow_html=True)
            for i in range(8):
                px  = round(base + (i+1)*base*0.0005, 2)
                sz  = random.randint(100,5000)
                tot = px * sz
                st.markdown(f'<div class="ob-row"><span class="ob-ask">₹{px:,.2f}</span><span style="color:#E8E8E8;">{sz:,}</span><span class="ob-vol">₹{tot:,.0f}</span></div>', unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)

        st.markdown(f"""
        <div style="text-align:center;background:var(--s2);border:1px solid var(--border);border-radius:10px;padding:1rem;margin-top:1rem;">
          <div style="font-size:.65rem;color:#555;letter-spacing:2px;margin-bottom:4px;">LAST PRICE</div>
          <div style="font-family:'Bebas Neue',sans-serif;font-size:2rem;color:var(--gold);">₹{base:,.2f}</div>
        </div>""", unsafe_allow_html=True)

    with tab_wl:
        st.markdown('<div style="font-family:\'Bebas Neue\',sans-serif;font-size:1rem;letter-spacing:2px;color:#555;margin-bottom:1rem;">MY WATCHLIST</div>', unsafe_allow_html=True)
        all_syms = [s for g in SYMBOLS.values() for s in g]

        c1,c2 = st.columns([3,1])
        with c1:
            add_sym = st.selectbox("Add symbol to watchlist", [s for s in all_syms if s not in st.session_state.watchlist], key="wl_add")
        with c2:
            st.markdown("<br>", unsafe_allow_html=True)
        ...
        st.markdown('<div style="background:var(--s1);border:1px solid var(--border);border-radius:12px;padding:.5rem 1rem;">', unsafe_allow_html=True)
        for sym in st.session_state.watchlist:
            ...
            st.markdown(f"""
            <div class="wl-row">
              <div><div style="font-weight:700;color:#E8E8E8;">{sym}</div><div style="font-size:.7rem;color:#555;">{data['vol']} volume</div></div>
              <div style="font-family:'JetBrains Mono',monospace;color:#E8E8E8;">₹{data['price']:,.2f}</div>
              <div style="font-weight:700;color:{col};">{sign}{chg:.2f}%</div>
            </div>""", unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════
    ...
    uid  = st.session_state.user["id"]
    name = st.session_state.user.get("name","Trader")
    sec("📊 TRADE ANALYTICS","Deep dive into your trading performance")
    ...
    if not all_trades:
        st.markdown('<div style="text-align:center;padding:4rem;color:#444;background:var(--s1);border:1px solid var(--border);border-radius:12px;">No trades to analyse yet — make some trades first!</div>', unsafe_allow_html=True)
        footer(); st.stop()
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:1rem;margin-bottom:1.5rem;">
      <div class="m-card"><div class="m-label">Profit Factor</div><div class="m-val {'g' if profit_factor>=1.5 else 'r'}">{profit_factor}x</div><div class="m-sub">{'✅ Strong edge' if profit_factor>=1.5 else '⚠️ Needs work'}</div></div>
    ...
    for t in all_trades:
        ...
        sym_pnl[s]  = sym_pnl.get(s,0) + p
        sym_cnt[s]  = sym_cnt.get(s,0) + 1
        sym_wins[s] = sym_wins.get(s,0) + (1 if p>0 else 0)

    st.markdown('<div style="font-family:\'Bebas Neue\',sans-serif;font-size:1rem;letter-spacing:2px;color:#555;margin:.5rem 0 1rem;">P&L BREAKDOWN BY SYMBOL</div>', unsafe_allow_html=True)
    st.markdown('<div style="background:var(--s1);border:1px solid var(--border);border-radius:12px;overflow:hidden;"><div style="display:grid;grid-template-columns:1.5fr 1fr 1fr 1fr;padding:.8rem 1.2rem;background:var(--s2);font-size:.65rem;color:#555;letter-spacing:1.5px;text-transform:uppercase;"><span>SYMBOL</span><span>TOTAL P&L</span><span>TRADES</span><span>WIN RATE</span></div>', unsafe_allow_html=True)
    for sym, total in sorted(sym_pnl.items(), key=lambda x: x[1], reverse=True):
        pc  = "var(--green)" if total>=0 else "var(--red)"
        ps2 = "+" if total>=0 else ""
        cnt = sym_cnt.get(sym,1)
        wr2 = round(sym_wins.get(sym,0)/cnt*100,0)
        wrc2= "var(--green)" if wr2>=50 else "var(--red)"
        st.markdown(f'<div style="display:grid;grid-template-columns:1.5fr 1fr 1fr 1fr;padding:.7rem 1.2rem;border-top:1px solid var(--border2);align-items:center;"><span style="font-weight:700;color:#E8E8E8;">{sym}</span><span style="color:{pc};font-weight:700;font-family:\'JetBrains Mono\',monospace;">{ps2}₹{total:,.0f}</span><span style="color:#E8E8E8;">{cnt}</span><span style="color:{wrc2};font-weight:700;">{wr2:.0f}%</span></div>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)

    # Direction analysis
    st.markdown('<div style="font-family:\'Bebas Neue\',sans-serif;font-size:1rem;letter-spacing:2px;color:#555;margin:1rem 0 .8rem;">BUY vs SELL ANALYSIS</div>', unsafe_allow_html=True)
    ...
    with col1:
        ...
        st.markdown(f'<div class="m-card"><div class="m-label">📈 BUY Trades ({len(buys)})</div><div class="m-val {pc}">{"+"if buy_pnl>=0 else ""}₹{buy_pnl:,.0f}</div><div class="m-sub">Total P&L from long trades</div></div>', unsafe_allow_html=True)
    with col2:
        pc = "g" if sell_pnl >= 0 else "r"
        st.markdown(f'<div class="m-card"><div class="m-label">📉 SELL Trades ({len(sells)})</div><div class="m-val {pc}">{"+"if sell_pnl>=0 else ""}₹{sell_pnl:,.0f}</div><div class="m-sub">Total P&L from short trades</div></div>', unsafe_allow_html=True)
    ...
    if challenge and account:
        ...
        <div style="background:linear-gradient(135deg,#130f00,var(--s1));border:2px solid var(--gold);border-radius:16px;padding:1.8rem 2rem;margin-bottom:1.5rem;">
          <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:1.5rem;">
            <div><div style="font-size:.62rem;color:#555;letter-spacing:2px;text-transform:uppercase;margin-bottom:4px;">Plan</div><div style="font-family:'Bebas Neue',sans-serif;font-size:1.6rem;color:var(--gold);">{challenge['plan'].upper()}</div></div>
            <div><div style="font-size:.62rem;color:#555;letter-spacing:2px;text-transform:uppercase;margin-bottom:4px;">Balance</div><div style="font-family:'Bebas Neue',sans-serif;font-size:1.6rem;color:#E8E8E8;">₹{bal:,.0f}</div></div>
            <div><div style="font-size:.62rem;color:#555;letter-spacing:2px;text-transform:uppercase;margin-bottom:4px;">P&L</div><div style="font-family:'Bebas Neue',sans-serif;font-size:1.6rem;color:{pc};">{ps}₹{p:,.0f}</div></div>
            <div><div style="font-size:.62rem;color:#555;letter-spacing:2px;text-transform:uppercase;margin-bottom:4px;">Progress</div><div style="font-family:'Bebas Neue',sans-serif;font-size:1.6rem;color:{'var(--green)' if pp>=r['target'] else 'var(--gold)'};">{ps}{pp:.1f}% / +{r['target']}%</div></div>
          </div>
        </div>""", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════
    ...
    if entries:
        for e in entries:
            out = e.get("outcome","")
            tc  = "win" if out=="win" else ("loss" if out=="loss" else "")
...
# ══════════════════════════════════════════════════════════════
    ...
    if not all_ch:
        ...
        total  = len(all_ch)
        passed = sum(1 for c in all_ch if c.get("status")=="passed")
        failed = sum(1 for c in all_ch if c.get("status")=="failed")
        active = sum(1 for c in all_ch if c.get("status")=="active")
        st.markdown(f"""
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:1rem;margin-bottom:1.5rem;">
...
# ══════════════════════════════════════════════════════════════
    ...
    for i,t in enumerate(data):
        ...
        bc    = "funded-b" if funded else "active-b"
        bt    = "FUNDED" if funded else "ACTIVE"
...
# ══════════════════════════════════════════════════════════════
    ...
    with st.form("edit_profile"):
        ...
        if submitted:
            if db_update_profile(uid, new_name, new_country, new_bio):
                ...
                push_notification(uid,"✅","Profile Updated","Your profile has been saved.")
                st.success("✅ Profile saved!"); time.sleep(1); st.rerun()
            else:
                st.error("❌ Save failed — ensure 'bio' column exists in profiles table.")

    st.markdown("<br>", unsafe_allow_html=True)
    col_a, col_b = st.columns(2)
    with col_a:
        st.markdown(f'<div class="m-card"><div class="m-label">Email</div><div style="font-size:.9rem;color:#E8E8E8;margin-top:4px;">{email}</div><div class="m-sub">Supabase Auth account</div></div>', unsafe_allow_html=True)
    with col_b:
        st.markdown(f'<div class="m-card"><div class="m-label">Member Since</div><div style="font-family:\'Bebas Neue\',sans-serif;font-size:1.4rem;color:var(--gold);">{all_challenges[-1].get("started_at","")[:7] if all_challenges else "NEW"}</div><div class="m-sub">First challenge date</div></div>', unsafe_allow_html=True)
    ...
    if st.button("📧 SEND TEST EMAIL", key="test_email"):
        ok = send_email_notification(email, "✅ AKFunded Email Test", f"""
        <div style="font-family:Arial,sans-serif;background:#070707;color:#E8E8E8;padding:2rem;border-radius:12px;max-width:500px;">
          <h2 style="color:#F0B429;letter-spacing:3px;">AKFUNDED ⚡</h2>
          <h3>Email is working! ✅</h3>
          <p style="color:#666;">Hey {name}, your SMTP is configured correctly.</p>
          <p style="font-size:.75rem;color:#333;">Powered by Akash Injeti</p>
        </div>""")
        if ok:
            st.success(f"✅ Test email sent to {email}!")
        else:
            st.error("❌ Failed — check SMTP_EMAIL and SMTP_PASSWORD in secrets.")
        ...
        for n in notifs:
            ...
            <div class="notif-item {unread_class}">
              ...
              <div class="notif-body">
                <div class="notif-title">{n.get('title','Notification')}{'<span class="notif-badge">NEW</span>' if n.get('unread') else ''}</div>
...
# ══════════════════════════════════════════════════════════════
    ...
    st.markdown('<div style="background:var(--s1);border:1px solid var(--border);border-radius:12px;overflow:hidden;"><div class="admin-row header"><span>TRADER</span><span>PLAN</span><span>BALANCE</span><span>P&L %</span><span>DAYS</span><span>STATUS</span></div>', unsafe_allow_html=True)
    for r in filtered[:50]:
        ...
        st.markdown(f"""
        <div class="admin-row">
          <div><div style="font-weight:600;color:#E8E8E8;">{r['name']}</div><div style="font-size:.7rem;color:#555;">{r['email']}</div></div>
...
The trader: {name}, on the {plan.upper()} challenge. P&L: {pnl_pct:.1f}%. Balance: ₹{balance:,.0f}.
Recent trades: {[t.get('symbol','') + ' ' + t.get('type','') + ' P&L ₹' + str(t.get('pnl',0)) for t in trades[:5]]}.
Give concise, practical advice on: risk management, passing the challenge, market psychology, technical analysis for Indian markets.
Keep replies under 150 words. Be direct and encouraging."""

    sec("🤖 AI TRADING ASSISTANT","Your personal trading coach — powered by Groq & LLaMA 3.3 70B")
    ...
    if not chat:
        ...
        <div class="chat-messages">
          <div class="chat-msg">
            ...
            <div class="chat-bubble ai">Hey {name}! 👋 I'm your AKFunded trading coach. Ask me anything — risk management, setups, drawdown recovery, or tips to pass your challenge faster!</div>
          </div>
        </div>""", unsafe_allow_html=True)
    else:
        msgs_html = ""
    ...
    with col1:
        ...
        <div class="risk-card" style="margin-top:1rem;">
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;">
            <div class="risk-result"><div style="font-size:.62rem;color:#555;letter-spacing:2px;margin-bottom:4px;">POSITION SIZE</div><div class="risk-val" style="color:var(--gold);">{qty:,}</div><div style="font-size:.7rem;color:#555;">shares / lots</div></div>
            <div class="risk-result"><div style="font-size:.62rem;color:#555;letter-spacing:2px;margin-bottom:4px;">RISK AMOUNT</div><div class="risk-val" style="color:var(--red);">₹{risk_amt:,.0f}</div><div style="font-size:.7rem;color:#555;">{risk_pct}% of account</div></div>
            <div class="risk-result"><div style="font-size:.62rem;color:#555;letter-spacing:2px;margin-bottom:4px;">REWARD</div><div class="risk-val" style="color:var(--green);">₹{reward_amt:,.0f}</div><div style="font-size:.7rem;color:#555;">if target hit</div></div>
            <div class="risk-result"><div style="font-size:.62rem;color:#555;letter-spacing:2px;margin-bottom:4px;">R:R RATIO</div><div class="risk-val" style="color:{rc};">1:{rr_ratio:.1f}</div><div style="font-size:.7rem;color:#555;">{'✅ Good' if rr_ratio>=2 else '⚠️ Low' if rr_ratio>=1 else '❌ Poor'}</div></div>
          </div>
          <div style="margin-top:1rem;padding:.8rem;background:var(--s2);border-radius:8px;font-size:.78rem;color:#666;">
            ...
            SL: <b style="color:var(--red);">₹{sl_dist:.2f}</b> &nbsp;·&nbsp;
            TP: <b style="color:var(--green);">₹{tp_dist:.2f}</b>
          </div>
        </div>""", unsafe_allow_html=True)
        ...
        if challenge and account:
            ...
            <div class="risk-card">
              <div style="margin-bottom:1rem;">
                <div style="font-size:.65rem;color:#555;letter-spacing:2px;text-transform:uppercase;margin-bottom:4px;">Daily Loss Remaining</div>
              ...
              <div style="margin-bottom:1rem;">
                <div style="font-size:.65rem;color:#555;letter-spacing:2px;text-transform:uppercase;margin-bottom:4px;">Total Loss Remaining</div>
              ...
              <div>
                <div style="font-size:.65rem;color:#555;letter-spacing:2px;text-transform:uppercase;margin-bottom:4px;">Days Still Needed</div>
...
# ══════════════════════════════════════════════════════════════
    ...
    if not passed_ch:
        ...
        st.markdown(render_certificate(name, ch["plan"], cap, pnl_pct, days, date_str), unsafe_allow_html=True)
        st.markdown("<br>", unsafe_allow_html=True)
        col1, col2 = st.columns(2)
        with col1:
            if st.button("📧 EMAIL MY CERTIFICATE", use_container_width=True, key="email_cert"):
                email    = st.session_state.user.get("email","")
                cap_str  = f"₹{cap//100000}L" if cap>=100000 else f"₹{cap//1000}K"
                html_body= f"""<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#070707;color:#E8E8E8;padding:2rem;border-radius:12px;">
                  ...
                  <p style="text-align:center;color:#666;">You passed the <b style="color:#F0B429;">{ch['plan'].upper()} Challenge ({cap_str})</b></p>
                  <div style="background:#111;border-radius:12px;padding:1.5rem;margin:1.5rem 0;text-align:center;">
                    <div style="font-size:2rem;color:#00C896;font-weight:700;">+{pnl_pct:.2f}%</div>
                    <div style="color:#666;font-size:.85rem;">Profit in {days} trading days</div>
                  </div>
                ...
                if ok: st.success(f"✅ Certificate emailed to {email}")
                else:  st.info("📧 Add SMTP_EMAIL & SMTP_PASSWORD to secrets to enable email.")
        with col2:
...
# ══════════════════════════════════════════════════════════════
    ...
    with c3:
        if st.button("📧 EMAIL INVITE", use_container_width=True, key="email_invite"):
            html_invite = f"""<div style="font-family:Arial,sans-serif;max-width:600px;background:#070707;color:#E8E8E8;padding:2rem;border-radius:12px;">
              <h1 style="color:#F0B429;letter-spacing:4px;">AKFUNDED ⚡</h1>
              <h2>Hey! {name} invites you to trade 🚀</h2>
              <p style="color:#666;">Join AKFunded — India's premier prop trading simulator.</p>
              ...
              <p style="color:#666;"><a href="{share_url}" style="color:#F0B429;">Click here to join</a></p>
            </div>"""
            ok = send_email_notification(email, f"{name} invited you to AKFunded! 🚀", html_invite)
            if ok: st.success("✅ Invite sent!")
            else:  st.info("📧 Configure SMTP_EMAIL & SMTP_PASSWORD in secrets to send emails.")
