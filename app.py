...
<style>
  #globe-wrap { width:100%; display:flex; align-items:center; justify-content:center; gap:3rem; padding:1rem 0 2rem; background:transparent; }
  #globeCanvas { display:block; }
  .globe-stats { display:flex; flex-direction:column; gap:1.2rem; }
  .gstat { background:#0d0d0d; border:1px solid #1e1e1e; border-left:2px solid #00D4FF; padding:.9rem 1.4rem; min-width:200px; }
  .gstat-val { font-family:'Courier New',monospace; font-size:1.6rem; font-weight:700; color:#00D4FF; letter-spacing:2px; }
  .gstat-lbl { font-size:.58rem; color:#505050; letter-spacing:2.5px; text-transform:uppercase; margin-top:3px; }
  .gstat-val.g { color:#00B87A; }
  .gstat-val.o { color:#D4A843; }
</style>
<div id="globe-wrap">
...
(function(){
  ...
  // Generate random trader nodes on sphere surface
  const nodes = Array.from({length:60},()=>{
    const theta = Math.random()*Math.PI*2;
    const phi   = Math.acos(2*Math.random()-1);
    return { theta, phi,
      color: Math.random()>0.5?'#00D4FF':Math.random()>0.5?'#00B87A':'#D4A843',
  ...
  function project(theta, phi, rot){
    ...
    return { sx: cx+x*scale*1.1, sy: cy-y*scale*1.1, z, scale };
  }
  ...
  function draw(){
    ...
    ctx.fillStyle=grad; ctx.beginPath(); ctx.arc(cx,cy,R*1.15,0,Math.PI*2); ctx.fill();
    ...
    lines.forEach(({i,j})=>{
      ...
      if(a.z>-R*0.4&&b.z>-R*0.4){
        ctx.beginPath(); ctx.moveTo(a.sx,a.sy); ctx.lineTo(b.sx,b.sy);
        ctx.strokeStyle='rgba(0,212,255,0.12)'; ctx.lineWidth=0.7; ctx.stroke();
      }
    });
    ...
    nodes.forEach(n=>{
      ...
      ctx.beginPath();
      ctx.arc(p.sx,p.sy,n.size*p.scale,0,Math.PI*2);
      ctx.fillStyle=n.color.replace(')',`,${alpha*0.9})`).replace('rgb','rgba').replace('#','rgba(').replace('00D4FF','0,212,255').replace('00B87A','0,184,122').replace('D4A843','212,168,67');
      ...
      // glow
      if(alpha>0.6){
        ctx.beginPath(); ctx.arc(p.sx,p.sy,n.size*p.scale*2.5,0,Math.PI*2);
